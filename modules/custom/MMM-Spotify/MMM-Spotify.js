...
Module.register("MMM-Spotify", {
    ...
    */
	...
	updateShuffle: function (shuffleState) {
		...
		shuffle.className = shuffleState ? "on" : "off";

		const icon = shuffleState ? "mdi:shuffle" : "mdi:shuffle-disabled";

		shuffle.innerHTML = "";
		shuffle.appendChild(this.getIconContainer("iconify", "SPOTIFY_CONTROL_SHUFFLE_ICON", icon));
	},
	...
	updateRepeat: function (repeatState) {
		...
		repeat.innerHTML = "";
		repeat.appendChild(this.getIconContainer("iconify", null, ris[repeatState]));
	},
	...
	updatePlaying: function (isPlaying) {
		...
		if (this.config.control !== "hidden") {
			...
			p.innerHTML = "";
			p.appendChild(this.getIconContainer("iconify", "SPOTIFY_CONTROL_PLAY_ICON", icon));
		}
	...
	clickPlay: function () {
		this.currentPlayback.is_playing ? this.sendSocketNotification("PAUSE") : this.sendSocketNotification("PLAY");
	},
	...
	clickBackward: function () {
		this.currentPlayback.progress_ms < 3000 ? this.sendSocketNotification("PREVIOUS") : this.sendSocketNotification("REPLAY");
	},
	...
	getDeviceContainer() {
		...
		device.appendChild(this.getIconContainer(this.getFAIconClass("default"), "SPOTIFY_DEVICE_ICON"));
	...
	getVolumeContainer() {
		...
		volume.appendChild(this.getIconContainer(this.getFAIconClass("VOL_OFF"), "SPOTIFY_VOLUME_ICON"));
	...
	getSpotifyLogoContainer() {
		...
		logo.appendChild(this.getIconContainer(this.getFAIconClass("Spotify"), "SPOTIFY_LOGO_ICON"));
		const text = document.createElement("span");
		text.className = "text";
	...
	getControlsContainer() {
		...
		for (const [key, config] of Object.entries(orderedButtonConfig)) {
			control.appendChild(this.getControlButton(key, config["icon"], config["action"]));
		}
	...
	getMinimalistBarDom: function (container) {
		...
		const infoElements = ["SPOTIFY_TITLE", "SPOTIFY_ALBUM", "SPOTIFY_ARTIST"];
		...
		if (this.config.control !== "hidden") {
			foreground.appendChild(
				this.getControlButton("SPOTIFY_CONTROL_PLAY", "mdi:play-circle-outline", () => {
					this.clickPlay();
				})
			);
		}
